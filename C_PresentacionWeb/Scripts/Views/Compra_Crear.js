...
$(document).ready(function () {
    ...
    //OBTENER TIENDAS
    tablatienda = $('#tbAlmacen').DataTable({
        "ajax": {
...
function buscarProducto() {
    if (parseInt($("#txtIdAlmacen").val()) == 0) {
        swal("Mensaje", "Debe seleccionar una tienda primero", "warning")
        return;
    }
    tablaproducto.ajax.url($.MisUrls.url._ObtenerProductosPorTienda + "?IdTienda=" + $("#txtIdAlmacen").val()).load();
...
function tiendaSelect(json) {
    $("#txtIdAlmacen").val(json.IdTienda);
    $("#txtRucAlmacen").val(json.RUC);
    $("#txtNombreAlmacen").val(json.Nombre);
...
$('#btnTerminarGuardarCompra').on('click', function () {
    ...
    compra = "<COMPRA>" +
        ...
        "<IdTienda>" + $("#txtIdAlmacen").val() + "</IdTienda>" +
