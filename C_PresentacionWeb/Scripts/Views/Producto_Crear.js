...
$(document).ready(function () {
    ...
    //OBTENER MARCAPROD
    jQuery.ajax({
        url: $.MisUrls.url._ObtenerMarcaProds,
        type: "GET",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            ...
            if (data.data != null) {
                $.each(data.data, function (i, item) {
                    ...
                    if (item.Activo == true) {
                        $("<option>").attr({ "value": item.IdMarcaProd }).text(item.Descripcion).appendTo("#cboMarcaProd");
                    }
                })
                $("#cboMarcaProd").val($("#cboMarcaProd option:first").val());
            }
    ...
    //OBTENER MEDIDA
    jQuery.ajax({
        url: $.MisUrls.url._ObtenerMedidas,
        type: "GET",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            ...
            if (data.data != null) {
                $.each(data.data, function (i, item) {
                    ...
                    if (item.Activo == true) {
                        $("<option>").attr({ "value": item.IdMedida }).text(item.Descripcion).appendTo("#cboMedida");
                    }
                })
                $("#cboMedida").val($("#cboMedida option:first").val());
            }
    ...
    tabladata = $('#tbdata').DataTable({
        ...
        "columns": [
            ...
            { "data": "Descripcion" },
            {
                "data": "oMarcaProd", render: function (data) {
...
function abrirPopUpForm(json) {
    ...
    if (json != null) {
        ...
        $("#cboMarcaProd").val(json.IdMarcaProd);
        $("#cboMedida").val(json.IdMedida);
    ...
    if ($("#form").valid()) {
        ...
        var request = {
            objeto: {
                ...
                IdMarcaProd: $("#cboMarcaProd").val(),
                IdMedida: $("#cboMedida").val(),
